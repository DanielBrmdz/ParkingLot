    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getActionCommand().equals("add")) {
            if (frIn.getPrice() > 0.0 || frIn.getPlate().toString() != "" || frIn.FoodCom().toString() != null) {
                if (frIn.TypeCom().getSelectedItem().equals("Tuberculo")) {
                    parking.addCar(frIn.getWeight(), frIn.getPlate(), parking.getProductTub(frIn.getProduc()));
                } else if (frIn.TypeCom().getSelectedItem().equals("Fruta")) {
                    parking.addCar(frIn.getWeight(), frIn.getPlate(), parking.getProductFru(frIn.getProduc()));
                } else if (frIn.TypeCom().getSelectedItem().equals("Hortaliza")) {
                	
                    parking.addCar(frIn.getWeight(), frIn.getPlate(), parking.getProductVeg(frIn.getProduc()));
                }
            } else {
